...
const GalleryManager = () => {
  ...
  const copyToClipboard = (url) => {
    const fullUrl = url.startsWith('http') ? url : `${process.env.REACT_APP_API_URL?.replace('/api', '') || 'http://localhost:10000'}${url}`;
            ...
            filteredGallery.map((img) => (
              ...
              >
                <div className="h-48 overflow-hidden relative bg-gray-100 dark:bg-gray-800">
                  <img
                    src={img.image && img.image.startsWith('http') ? img.image : `${process.env.REACT_APP_API_URL?.replace('/api', '') || 'http://localhost:10000'}${img.image}`}
